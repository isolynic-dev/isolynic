...
import { useAccount, useAuth, useSavedConfirmation } from '@/hooks/hooks';
...
// ---------------------------------------------------------------------------
// ProtectionSummarySection.tsx  (NEW — blueprint item 35)
// ---------------------------------------------------------------------------
//
// Deliberately minimal: a quiet reminder of accumulated value, not a
// marketing banner. Lives directly under the existing StatusBanner.

const CHANNEL_TITLES: Record<keyof ChannelsState, string> = {
  whatsapp: 'WhatsApp',
  phone: 'Phone',
  website: 'Website',
};

function ProtectionSummarySection({
  channels,
  ...
  const connectedKeys = (Object.keys(channels) as (keyof ChannelsState)[]).filter(
    (key) => channels[key].status === 'connected'
  );

  if (connectedKeys.length === 0) return null;

  return (
    <AccountCard title="Isolynic is currently protecting">
      <p className="text-sm text-gray-900">
        Your business is currently protected across{' '}
        <span className="font-medium">
          {connectedKeys.map((key) => CHANNEL_TITLES[key]).join(', ')}
        </span>
        {' '}— {connectedKeys.length} customer {connectedKeys.length === 1 ? 'channel' : 'channels'}.
      </p>
      <p className="text-sm text-gray-500">
        {opportunitiesRecovered} {opportunitiesRecovered === 1 ? 'opportunity' : 'opportunities'} recovered since activation.
      </p>
    </AccountCard>
  );
}
...
function BusinessSection({ uid, business }: { uid: string; business: BusinessInfo }) {
...
function ChannelsSection({ channels }: { channels: ChannelsState }) {
...
function BusinessHoursSection({ uid, hours }: { uid: string; hours: BusinessHoursState }) {
  const [editing, setEditing] = useState(false);
...
function CalendarSection({ uid, calendar }: { uid: string; calendar: CalendarState }) {
...
// ---------------------------------------------------------------------------
// RecoveryPreferencesSection.tsx
// ---------------------------------------------------------------------------
...
function RecoveryPreferencesSection({
  uid,
  recovery,
}: {
  uid: string;
...
// ---------------------------------------------------------------------------
// BillingSection.tsx  (MODIFIED — blueprint item 36)
// ---------------------------------------------------------------------------
//
// The plan is now shown attached to demonstrated value:
//   Growth
//   Protecting your customer opportunities
//   7 recovered since activation
//   [View plan]
// Usage/renewal/past-due detail stays available underneath rather than
// replaced, since it's still functionally necessary.

function BillingSection({ subscription }: { subscription: SubscriptionState }) {
  const [busy, setBusy] = useState(false);
  ...
  return (
    <AccountCard title="Plan & billing" subtitle="Manage your Isolynic subscription.">
      <div>
        ...
        <p className="text-sm text-gray-500">Protecting your customer opportunities</p>
        <p className="text-sm text-gray-500">
          {subscription.usage.opportunitiesRecovered}{' '}
          {subscription.usage.opportunitiesRecovered === 1 ? 'opportunity' : 'opportunities'} recovered since activation
        </p>
      </div>
...
// ---------------------------------------------------------------------------
// HelpAndControlsSection.tsx
// ---------------------------------------------------------------------------

function HelpAndControlsSection({ runState }: { runState: IsolynicRunState }) {
  const [busy, setBusy] = useState(false);
...
function StatusBanner({ account }: { account: AccountState }) {
...
// ---------------------------------------------------------------------------
// AccountPage (page.tsx default export)
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:py-8">
      ...
      <div className="mb-6 space-y-4">
        ...
        <ProtectionSummarySection
          channels={account.channels}
          opportunitiesRecovered={account.subscription.usage.opportunitiesRecovered}
        />
      </div>
